...
<%@page import="org.apache.taglibs.standard.tag.common.xml.ForEachTag"%>
		<div id="container">
			...
			<div class="sub_prdlist innerContent">
				<div class="sub_title">
					<a href=""><strong>이달의 신상품</strong></a>
				</div>
				${pageDTO.count }개
				<div class="item-display-wrap">
					<div class="item-display type-cart clear cboth">
						<div class="list innerContent">
							<!-- 상품목록 -->
							<ul>
							<c:forEach var="ListDTO" items="${getNewList }">
							<li class="line_4">
							...
							<div class="paging">
								<c:if test="${pageDTO.startPage > pageDTO.pageBlock }">
									<a href="${pageContext.request.contextPath}/sub_new?pageNum=${pageDTO.startPage-pageDTO.pageBlock}">[이전]</a>
								</c:if>
								<c:forEach var="i" begin="${pageDTO.startPage }" end="${pageDTO.endPage }" step="1">
									<a href="${pageContext.request.contextPath}/sub_new?pageNum=${i}">${i} </a>
								</c:forEach>
								<c:if test="${pageDTO.endPage < pageDTO.pageCount }">
									<a href="${pageContext.request.contextPath}/sub_new?pageNum=${pageDTO.startPage+pageDTO.pageBlock}">[다음]</a>
								</c:if>
							</div>
								<!-- //페이징 끝-->
						</div><!-- proslide01 -->
