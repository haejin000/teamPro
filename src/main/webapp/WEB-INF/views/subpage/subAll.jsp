...
<%@page import="org.apache.taglibs.standard.tag.common.xml.ForEachTag"%>
		<div id="container">
        	...
        	<div class="search_panel_wrapper Search Search_S">
				<div class="innerContent">	
					<div id="subcateList">
					<div id="subcatedepth1" class="ssSection">
						<div class="ssTitle"><span>카테고리 '대'</span></div><!--ssTitle-->
						<div class="ssCont">
							<div class="sBox"><a href="${pageContext.request.contextPath}/sub?c=&l=">All</a></div>
							<div class="sBox"><a href="${pageContext.request.contextPath}/sub?c=a&l=A" class="on">건강기능식품</a></div>
							<div class="sBox"><a href="${pageContext.request.contextPath}/sub?c=b&l=A">건강가공식품</a></div>			
							<div class="sBox"><a href="${pageContext.request.contextPath}/sub?c=c&l=A">잡화</a></div>					
							<div class="sBox"><a href="${pageContext.request.contextPath}/sub?c=d&l=A" >기타상품</a></div>				
							<div class="sBox"><a href="${pageContext.request.contextPath}/sub?c=e&l=A" >사은품</a></div>
						</div><!--ssCont-->
					</div><!--subcatedepth1-->
					<div id="subcatedepth2" class="ssSection">
						<div class="ssTitle"><span>카테고리 '중'</span></div><!--ssTitle-->
					<c:choose>
					<c:when test="#">
						<div class="ssCont">
							<div class="sBox"><a href="" ></a></div>
							<div class="sBox"><a href="" >프로바이오틱스</a></div>
							<div class="sBox"><a href="" >칼슘</a></div>
							<div class="sBox"><a href="" >마그네슘</a></div>
						</div><!--ssCont-->
					</c:when>	
					<c:when test="${pageContext.request.contextPath}/sub?c=b&l=A">
						<div class="ssCont">
							<div class="sBox"><a href="" >단백질</a></div>
							<div class="sBox"><a href="" >두유</a></div>
							<div class="sBox"><a href="" >닭가슴살</a></div>
							<div class="sBox"><a href="" >아연</a></div>
						</div><!--ssCont-->
					</c:when>
					<c:when test="${pageContext.request.contextPath}/sub?c=c&l=A">
						<div class="ssCont">
							<div class="sBox"><a href="" >생활잡화</a></div>
						</div><!--ssCont-->
					</c:when>	
					<c:when test="${pageContext.request.contextPath}/sub?c=d&l=A">
						<div class="ssCont">
							<div class="sBox"><a href="" >신선식품</a></div>
							<div class="sBox"><a href="" >차/티백</a></div>
						</div><!--ssCont-->
					</c:when>						
					</c:choose>
					</div><!--subcatedepth2-->
					</div>
						<form action="${pageContext.request.contextPath}/sub?c=&l=">
							<div id="subSearchForm" class="ssSection" style="display: none;">
								<div class="ssTitle">
									<span>가격</span>
								</div>
								<div class="ssCont">
									<input type="text" id="sprice" name="sprice" maxlength="8" placeholder="최저가"><span class="tilde">~</span><input type="text" id="eprice" name="eprice" maxlength="8" placeholder="최고가">
								</div>
								<div class="ssTitle">
									<span>결과 내 재검색</span>
								</div>
								<div class="ssCont">
									...
									<button class="btn_kwdSC iconfont ftic-search" onclick="">검색하기</button>
								</div>
							</div><!--subSearchForm-->
						</form>
...
<div class="goods-align">
	<div class="innerContent">
		<span class="tc">상품 <strong>78</strong> 개</span>
		<form name="frmList" action="">
			<input type="hidden" name="cateCd" value="009">
			<fieldset>
				<legend>상품 정렬 폼</legend>
				<div class="align">
					<ul>
						<li>
							<input type="radio" id="sort1" class="radio" name="searchType" value="orderCnt desc,reviewCnt desc">
								<!-- 주문많은순 리뷰많은순 -->
							<label for="sort1" class="on">판매인기순</label>
						</li>
						<li>
							<input type="radio" id="sort3" class="radio" name="searchType" value="goodsPrice,salesStartYmd desc"
							<c:if test="${searchType == 1}"> checked </c:if>><!-- 가격오름차순 판매일 내림차순 -->
							<label for="sort3">낮은가격순</label>
						</li>
						<li>
							<input type="radio" id="sort4" class="radio" name="searchType" value="goodsPrice desc,salesStartYmd desc"><!-- 가격내림차순 판매일 내림차순 -->
							<label for="sort4">높은가격순</label>
						</li>
						<li>
							<input type="radio" id="sort5" class="radio" name="searchType" value="reviewCnt desc,salesStartYmd desc"><!--리뷰 내림차순 판매일 내림차순 -->
							<label for="sort5">상품평순</label>
						</li>
						<li>
							<input type="radio" id="sort6" class="radio" name="searchType" value="salesStartYmd"><!-- 판매일 오름차순 -->
							<label for="sort6">등록일순</label>
						</li>
					</ul>
					<div class="choice">
						<div>
							<select name="pageSize" class="tune">
				...
				<div class="item-display-wrap">
					<div class="item-display type-cart clear cboth">
						<div class="list innerContent">
							<!-- 상품목록 -->
							<ul>
							<c:forEach var="ListDTO" items="${categoryList }">
							<li class="line_4">
							...
							<div class="paging">
								<c:if test="${pageDTO.startPage > pageDTO.pageBlock }">
									<a href="${pageContext.request.contextPath}/sub?c=&l=&keyword=&pageNum=${pageDTO.startPage-pageDTO.pageBlock}">[이전]</a>
								</c:if>
								<c:forEach var="i" begin="${pageDTO.startPage }" end="${pageDTO.endPage }" step="1">
									<a href="${pageContext.request.contextPath}/sub?c=&l=&keyword=&pageNum=${i}">${i}</a>
								</c:forEach>
								<c:if test="${pageDTO.endPage < pageDTO.pageCount }">
									<a href="${pageContext.request.contextPath}/sub?c=&l=&keyword=&pageNum=${pageDTO.startPage+pageDTO.pageBlock}">[다음]</a>
								</c:if>
							</div>
							<!-- //페이징 끝-->
						</div><!-- proslide01 -->
